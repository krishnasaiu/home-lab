...
if [ $# -lt 1 ]; then
    echo -e "${YELLOW}Usage: $0 [postgres | vaultwarden | all]${NC}"
    ...
    echo "  all         - Forwards both services in background tasks"
    exit 1
fi
...
case "$SERVICE_NAME" in
    ...
    all)
        ...
        echo -e "${YELLOW}[TIP] Press Ctrl+C to terminate both background forwards.${NC}"
        
        # Capture Ctrl+C and kill background forwards automatically
        trap "kill $PID_PG $PID_VW 2>/dev/null || true; echo -e '\n${GREEN}[+] Port forwards closed.${NC}'; exit" INT TERM EXIT
        wait
        ;;
    *)
        echo -e "${RED}[ERROR] Unknown service '$SERVICE_NAME'. Supported: postgres, vaultwarden, all.${NC}"
        exit 1
        ;;
esac
